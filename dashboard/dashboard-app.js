(() => {
  ...
  class DashboardApp {
    constructor({ rootElement, transport, pollMs = 3000, download = downloadText, confirmAction = null } = {}) {
    ...
    render() {
      if (this.lastError && !this.dashboard) {
        this.rootElement.innerHTML = `<div class="dashboard-error">Dashboard unavailable: ${escapeHtml(this.lastError)}</div>`;
        return;
      }
      const d = this.dashboard;
      if (!d) {
        this.rootElement.innerHTML = `<div class="dashboard-empty">Loading Dashboard…</div>`;
        return;
      }
      ...
      this.rootElement.innerHTML = `
        <div class="dashboard-shell">
          ...
          <div class="dashboard-topbar">
            <div>
              <h2>Dashboard</h2>
              <p>${project ? `${escapeHtml(project.status)} · ${escapeHtml(project.repository?.fullName || project.repository?.url || "repository")}` : "No active project"}</p>
            </div>
            <button class="secondary compact" data-dashboard-action="refresh">Refresh</button>
          </div>
          ...
          <div class="dashboard-toolbar">
            <button data-dashboard-action="pause" ${canPause ? "" : "disabled"}>Pause</button>
            <button data-dashboard-action="resume" ${canResume ? "" : "disabled"}>Resume</button>
            <button class="secondary" data-dashboard-action="stopNow" ${canStop ? "" : "disabled"}>Stop Now</button>
            <button class="secondary" data-dashboard-action="startIntegration" ${canIntegrate ? "" : "disabled"}>Start Integration</button>
            <button class="secondary" data-dashboard-action="exportProject" ${project ? "" : "disabled"}>Export Project</button>
            <button class="secondary" data-dashboard-action="exportDebug">Export Debug</button>
          </div>
    ...
    renderProject(project, d) {
      ...
      return `<section class="dashboard-section">
        ...
        <div class="dashboard-kv">
          <span>Repository</span><strong>${escapeHtml(project.repository?.url || "—")}</strong>
    ...
    renderWarnings(warnings) {
      return `<section class="dashboard-section">
        ...
        <div class="dashboard-list">${warnings.length ? warnings.map((item) => `<div class="dashboard-warning severity-${escapeHtml(item.severity)}"><strong>${escapeHtml(item.code)}</strong><span>${escapeHtml(item.message)}</span><small>${formatTime(item.at)}</small></div>`).join("") : `<p class="dashboard-muted">No warnings at this filter.</p>`}</div>
      </section>`;
    }
    ...
    renderTasks(tasks, agents) {
      ...
      return `<section class="dashboard-section">
        <div class="dashboard-section-head"><h3>Tasks / DAG</h3><span>${tasks.length}</span></div>
        <div class="dashboard-task-list">${tasks.length ? tasks.map((task) => {
          const controls = task.controls || {};
          const dependencyText = task.dependencies?.length ? task.dependencies.join(", ") : "none";
          ...
          return `<details class="dashboard-task" ${["RUNNING", "NEEDS_USER", "REVIEWING"].includes(task.status) ? "open" : ""}>
            ...
            <div class="dashboard-task-body">
              ...
              <div class="dashboard-kv compact">
                <span>Dependencies</span><strong>${escapeHtml(dependencyText)}</strong>
                <span>Priority</span><strong>${escapeHtml(task.priority)}</strong>
                <span>Risk / size</span><strong>${escapeHtml(task.risk)} / ${escapeHtml(task.estimatedComplexity)}</strong>
                <span>Run</span><strong>${escapeHtml(task.activeRunId || task.lastRunId || "—")}</strong>
                <span>Artifact</span><strong>${artifact ? `${escapeHtml(artifact.branch || "branch")}@${escapeHtml(shortSha(artifact.commit))}` : "—"}</strong>
              </div>
              ${task.acceptanceCriteria?.length ? `<div class="dashboard-evidence"><strong>Acceptance</strong><ul>${task.acceptanceCriteria.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></div>` : ""}
              ${task.lastReview ? `<div class="dashboard-evidence"><strong>Last review</strong><pre>${escapeHtml(jsonText(task.lastReview))}</pre></div>` : ""}
              ${task.lastError ? `<div class="dashboard-evidence"><strong>Last error</strong><pre>${escapeHtml(jsonText(task.lastError))}</pre></div>` : ""}
              <div class="dashboard-task-controls">
                <button data-dashboard-action="retryTask" data-task-id="${escapeHtml(task.taskId)}" ${controls.canRetry ? "" : "disabled"}>Retry</button>
                <button class="secondary" data-dashboard-action="cancelTask" data-task-id="${escapeHtml(task.taskId)}" ${controls.canCancel ? "" : "disabled"}>Cancel</button>
                <button class="secondary" data-dashboard-action="requestReview" data-task-id="${escapeHtml(task.taskId)}" ${controls.canRequestReview ? "" : "disabled"}>Review</button>
                <label class="dashboard-inline-control">Priority <input type="number" min="-100" max="100" value="${escapeHtml(task.priority)}" data-priority-task="${escapeHtml(task.taskId)}" ${controls.canChangePriority ? "" : "disabled"}></label>
                <button class="secondary" data-dashboard-action="changePriority" data-task-id="${escapeHtml(task.taskId)}" ${controls.canChangePriority ? "" : "disabled"}>Set</button>
                <select data-reassign-task="${escapeHtml(task.taskId)}" ${controls.canReassign && idleWorkers.length ? "" : "disabled"}><option value="">Worker…</option>${idleWorkers.map((agent) => `<option value="${escapeHtml(agent.agentId)}">${escapeHtml(agent.label || agent.agentId)}</option>`).join("")}</select>
                <button class="secondary" data-dashboard-action="reassignAgent" data-task-id="${escapeHtml(task.taskId)}" ${controls.canReassign && idleWorkers.length ? "" : "disabled"}>Assign</button>
              </div>
            </div>
          </details>`;
        }).join("") : `<p class="dashboard-muted">Task graph not available yet.</p>`}</div>
      </section>`;
    }

    renderAgents(agents) {
      return `<section class="dashboard-section">
        <div class="dashboard-section-head"><h3>Agent health</h3><span>${agents.length}</span></div>
        <div class="dashboard-agent-grid">${agents.length ? agents.map((agent) => `<div class="dashboard-agent"><div><strong>${escapeHtml(agent.label || agent.agentId)}</strong><small>${escapeHtml(agent.role)} · seen ${formatTime(agent.lastSeenAt)}</small></div>${statusBadge(agent.connected ? agent.status : "OFFLINE")}<button class="secondary compact" data-dashboard-action="openExecutor" data-agent-id="${escapeHtml(agent.agentId)}" ${agent.connected ? "" : "disabled"}>Open</button></div>`).join("") : `<p class="dashboard-muted">No agents registered.</p>`}</div>
      </section>`;
    }

    renderReviews(reviews) {
      const items = reviews.items || [];
      return `<section class="dashboard-section"><div class="dashboard-section-head"><h3>Review evidence</h3><span>${items.length}</span></div><div class="dashboard-list">${items.length ? [...items].reverse().slice(0, 20).map((review) => `<details class="dashboard-evidence"><summary>${statusBadge(review.status)} ${escapeHtml(review.taskId)} · ${escapeHtml(review.reviewId)}</summary><pre>${escapeHtml(jsonText(review.result || { reviewerAgentId: review.reviewerAgentId, lastError: review.lastError }))}</pre></details>`).join("") : `<p class="dashboard-muted">No reviews yet.</p>`}</div></section>`;
    }
    ...
    renderIntegration(integration) {
      ...
      return `<section class="dashboard-section"><div class="dashboard-section-head"><h3>Integration evidence</h3>${statusBadge(summary.status || "IDLE")}</div>
        ${current ? `<div class="dashboard-kv compact"><span>Run</span><strong>${escapeHtml(current.runId)}</strong><span>Agent</span><strong>${escapeHtml(current.agentId || "—")}</strong><span>Branch</span><strong>${escapeHtml(current.branch || "—")}</strong><span>Base</span><strong>${escapeHtml(shortSha(current.baseSha))}</strong></div>` : `<p class="dashboard-muted">No active integration run.</p>`}
        ${final ? `<details class="dashboard-evidence" open><summary>Verified summary</summary><pre>${escapeHtml(jsonText(final, 4000))}</pre></details>` : ""}
        ${(integration.repairs || []).length ? `<details class="dashboard-evidence"><summary>Repairs (${integration.repairs.length})</summary><pre>${escapeHtml(jsonText(integration.repairs, 4000))}</pre></details>` : ""}
      </section>`;
    }
    ...
    renderTimeline(events, rejections) {
      const rows = [
        ...
        ...rejections.map((record) => ({ at: record.receivedAt || record.at, kind: "rejection", label: `REJECTED · ${record.reason || "unknown"}`, detail: record.event || record.details || null }))
      ].sort((a, b) => (b.at || 0) - (a.at || 0)).slice(0, 60);
      return `<section class="dashboard-section"><div class="dashboard-section-head"><h3>Event timeline</h3><span>${rows.length}</span></div><div class="dashboard-timeline">${rows.length ? rows.map((row) => `<div class="dashboard-event ${row.kind}"><time>${formatTime(row.at)}</time><strong>${escapeHtml(row.label)}</strong><span>${escapeHtml(jsonText(row.detail, 600))}</span></div>`).join("") : `<p class="dashboard-muted">No events yet.</p>`}</div></section>`;
    }

    renderDecisions(decisions) {
      const rows = [...decisions].reverse().slice(0, 60);
      return `<section class="dashboard-section"><div class="dashboard-section-head"><h3>Scheduler explanation</h3><span>${rows.length}</span></div><div class="dashboard-timeline">${rows.length ? rows.map((item) => `<div class="dashboard-event"><time>${formatTime(item.at)}</time><strong>${escapeHtml(item.type)}</strong><span>${escapeHtml(jsonText(item.details, 900))}</span></div>`).join("") : `<p class="dashboard-muted">No scheduler decisions yet.</p>`}</div></section>`;
    }
    ...
    async handleClick(event) {
      ...
      if (action === "cancelTask") {
        ...
        if (first?.reason === "task_has_downstream_dependents" && this.confirmAction(`Cancel ${taskId} and downstream tasks: ${(first.dependentTaskIds || []).join(", ")}?`)) return this.command("cancelTask", { taskId, cascade: true });
