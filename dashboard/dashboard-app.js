  ...
  class DashboardApp {
    ...
    renderIntegration(integration) {
      ...
      return `<section class="dashboard-section"><div class="dashboard-section-head"><h3>${escapeHtml(this.tr("dashboard.integrationEvidence", "Integration evidence"))}</h3>${statusBadge(summary.status || "IDLE")}</div>
        ${current ? `<div class="dashboard-kv compact"><span>${escapeHtml(this.tr("dashboard.run", "Run"))}</span><strong>${escapeHtml(current.runId)}</strong><span>${escapeHtml(this.tr("dashboard.agent", "Agent"))}</span><strong>${escapeHtml(current.agentId || "—")}</strong><span>${escapeHtml(this.tr("dashboard.branch", "Branch"))}</span><strong>${escapeHtml(current.branch || "—")}</strong><span>${escapeHtml(this.tr("dashboard.base", "Base"))}</span><strong>${escapeHtml(shortSha(current.baseSha))}</strong></div>` : `<p class="dashboard-muted">${escapeHtml(this.tr("dashboard.noIntegration", "No active integration run."))}</p>`}
