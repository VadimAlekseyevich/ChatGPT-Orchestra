...
test("A01 evidence comment requires the clean-profile login/privacy PASS fields", () => {
...
test("GitHub comment validation fetches the exact permalinks and records auditable hashes", async () => {
...
test("GitHub comment validation fails closed on missing token or URL mismatch", async () => {
  await assert.rejects(
    validateEvidenceComments(["A01", A01_REF, "A11", A11_REF], { token: "", fetchImpl: async () => ({ ok: true }) }),
  ...
  await assert.rejects(
    validateEvidenceComments(["A01", A01_REF, "A11", A11_REF], { token: "test-token", fetchImpl }),
