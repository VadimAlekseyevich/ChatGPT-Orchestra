...
function fakeGitProvider({ validation = null } = {}) {
  return {
    ...
    async captureBase(project) {
      return { ok: true, snapshot: { provider: "test-git", repositoryFullName: project.repository.fullName, defaultBranch: "main", baseSha: BASE_SHA, capturedAt: 1, cleanupPolicy: "retain_until_review_or_manual_cleanup", lastCheckedAt: 1, currentTargetSha: BASE_SHA } };
    },
    ...
    async validateArtifact({ run }) {
      ...
      return { ok: true, artifact: { provider: "test-git", branch: run.git.branch, commit: COMMIT_SHA, baseSha: BASE_SHA, targetBranch: "main", changedFiles: [`src/${run.taskId}.js`], verifiedAt: 2 }, freshness: { ok: true, currentTargetSha: BASE_SHA, checkedAt: 2 } };
    }
  };
}
...
function task(id, dependencies, allow, priority = 50) {
  return { id, title: id, objective: `Execute ${id}`, kind: "code", dependencies, scope: { allow }, acceptanceCriteria: [`${id} accepted`], verification: ["npm test"], priority, risk: "low", estimatedComplexity: "S" };
}

function project(tasks) {
  return { projectId: "P1", status: "READY", initialGoal: "Execute a synthetic scheduler test safely.", repository: { url: "https://github.com/acme/widget", owner: "acme", repo: "widget", fullName: "acme/widget" }, taskGraph: { tasks } };
}
...
function completion(run) {
  return { event: { v: 1, event: "DONE", projectId: "P1", taskId: run.taskId, runId: run.runId, agentId: run.agentId, eventId: `E-${run.runId}`, sequence: 1, payload: { summary: "done", testsPerformed: ["npm test"], knownLimitations: [] } } };
}

function autoReviewEngine(store) {
  return {
    async init() {},
    async configureProject() {},
    async recoverReviewableTasks() {},
    getPublicState() { return { pending: 0, active: 0, total: 0 }; },
    activeReviewerAgentIds() { return []; },
    activeCount() { return 0; },
    async tick() { return { ok: true, assigned: [] }; },
    async handleAgentUnavailable() { return { ok: true, ignored: true }; },
    async handleAgentStateChanged() {},
    async enqueueForWorkerCompletion({ task, run, workerReport }) {
      const reviewId = `auto-review-${run.runId}`;
      await store.markReviewPending(task.id, reviewId, workerReport);
      await store.markReviewApproved(task.id, { reviewId, reviewerAgentId: "TEST_REVIEWER", workerRunId: run.runId, iteration: 1, result: { summary: "test auto approval" } });
...
function engineOptions({ store, projects, workers, idFactory, sendPrompt, clock, gitProvider } = {}) {
  return { store, projectStore: projects, registry: workers, eventBus: new FakeEventBus(), gitProvider: gitProvider || fakeGitProvider(), reviewEngine: autoReviewEngine(store), ...(clock ? { clock } : {}), ...(idFactory ? { idFactory } : {}), sendPrompt: sendPrompt || (async () => ({ ok: true })) };
}

test("runs three independent tasks in parallel and unlocks the dependent fourth only after reviewed prerequisites", async () => {
  const graph = project([task("T1", [], ["src/a/**"], 30), task("T2", [], ["src/b/**"], 20), task("T3", [], ["src/c/**"], 10), task("T4", ["T1", "T2", "T3"], ["src/final/**"], 100)]);
  const store = new SchedulerStore({ storageArea: fakeStorage() });
  const workers = registry(3);
  const projects = projectStore(graph);
  const prompts = [];
  let runId = 0;
  const engine = new SchedulerEngine(engineOptions({ store, projects, workers, idFactory: () => `R${++runId}`, sendPrompt: async (agentId, prompt) => { prompts.push({ agentId, prompt }); return { ok: true }; } }));
  ...
  assert.equal(store.summary().status, "READY_FOR_INTEGRATION");
  assert.equal(projects.state.project.status, "READY_FOR_INTEGRATION");
...
test("never assigns mutually exclusive overlapping tasks at the same time", async () => {
  const graph = project([task("T1", [], ["src/shared/**"], 100), task("T2", [], ["src/shared/nested/**"], 90)]);
  const store = new SchedulerStore({ storageArea: fakeStorage() });
  ...
  await engine.init(); await engine.start({ maxWorkers: 2 });
...
test("watchdog retries then escalates when retry budget is exhausted", async () => {
  ...
  now += 61000; await engine.tick({ reason: "test_watchdog" });
...
test("invalid Git artifact cannot unlock dependencies and is retried", async () => {
  ...
  const store = new SchedulerStore({ storageArea: fakeStorage() });
  const engine = new SchedulerEngine(engineOptions({ store, projects: projectStore(graph), workers: registry(1), idFactory: (() => { let id = 0; return () => `R${++id}`; })(), gitProvider: fakeGitProvider({ validation: () => ({ ok: false, reason: "git_branch_head_mismatch" }) }) }));
  await engine.init(); await engine.start({ maxWorkers: 1, maxRetries: 1 });
  const first = store.activeRuns()[0];
  await engine.handleCompletion(completion(first));
  assert.notEqual(store.getTask("T1").status, "APPROVED");
