(() => {
  ...
  class ObservabilityService {
    ...
    agents() {
      return (this.registry?.listAgents?.() || []).map((agent) => ({
        ...
        activeContext: agent.protocolContext ? clone(agent.protocolContext) : null,
    ...
    warnings({ minimumSeverity = "info" } = {}) {
      ...
      for (const issue of recovery?.issues || []) {
        output.push(warning(`recovery:${issue.code || output.length}`, "error", issue.code || "recovery_issue", issue.message || issue.code || "Recovery issue", issue, issue.at || recovery.updatedAt));
      }
      ...
      for (const rejection of recent.rejections || []) {
        output.push(warning(`event-rejection:${rejection.cursor || rejection.at || output.length}`, "warning", rejection.reason || "event_rejected", `Protocol event rejected: ${rejection.reason || "unknown"}`, { event: rejection.event || null }, rejection.receivedAt || rejection.at));
      }

      const minRank = severityRank(minimumSeverity);
      return output
        .filter((item) => severityRank(item.severity) >= minRank)
        .sort((a, b) => severityRank(b.severity) - severityRank(a.severity) || number(b.at) - number(a.at))
    ...
    metrics() {
      ...
      return {
        tasks: {
          ...
          cancelled,
          finished,
          progress: tasks.length ? finished / tasks.length : 0,
          byStatus: statusCounts
        },
        runs: {
          total: runs.length,
          active: runs.filter((run) => ["ASSIGNED", "RUNNING"].includes(run.status)).length,
          averageDurationMs: durations.length ? Math.round(durations.reduce((sum, item) => sum + item, 0) / durations.length) : 0
        },
        reviews: {
          total: reviews.length,
          active: reviews.filter((item) => ["ASSIGNED", "REVIEWING"].includes(item.status)).length,
          approved: reviews.filter((item) => item.status === "APPROVED").length,
          changesRequired: reviews.filter((item) => item.status === "CHANGES_REQUIRED").length
        },
        agents: {
          total: agents.length,
          connected: agents.filter((item) => item.connected).length,
          busy: agents.filter((item) => item.status === "BUSY").length,
          idle: agents.filter((item) => item.status === "IDLE").length,
    ...
    dashboard({ eventLimit = 60, decisionLimit = 60, minimumSeverity = "info" } = {}) {
      ...
      const repairs = clone(this.integrationStore?.listRepairs?.() || []);
      const recovery = this.recoveryController?.getPublicState?.() || null;
      const events = this.eventBus?.recent?.(eventLimit) || { events: [], rejections: [] };
      const decisions = this.schedulerStore?.recentDecisions?.(decisionLimit) || [];
      const persistence = typeof this.persistenceInfo === "function" ? this.persistenceInfo() : (this.persistenceInfo || {});
      ...
      const revision = Math.max(0, ...timestamps, number(events.events?.at?.(-1)?.receivedAt), number(events.rejections?.at?.(-1)?.receivedAt));
      ...
      return {
        ...
        project: project ? {
          ...
          stage: project.stage,
          goal: text(project.initialGoal || "", 12000),
          repository: clone(project.repository || null),
          validation: clone(project.validation || null),
          execution: clone(project.execution || null),
        ...
        reviews: {
          summary: this.reviewStore?.summary?.() || null,
          items: reviews
        },
        integration: {
          summary: integration,
        ...
        events: clone(events.events || []),
        rejections: clone(events.rejections || []),
        warnings: this.warnings({ minimumSeverity }),
        metrics: this.metrics(),
        persistence: {
          portableSchemaVersion: root.PortableState?.PORTABLE_SCHEMA_VERSION || 1,
          bundleVersion: root.ProjectBundle?.BUNDLE_VERSION || 1,
          ...clone(persistence)
        }
      };
    }
    ...
    debugBundle(options = {}) {
      ...
      return {
        ok: true,
        filename: `chatgpt-orchestra-debug-${dashboard.project?.projectId || "no-project"}.json`,
  ...
  if (typeof module !== "undefined" && module.exports) module.exports = { ObservabilityService, OBSERVABILITY_VERSION, taskPublic, runPublic, reviewPublic, integrationRunPublic };
})();
