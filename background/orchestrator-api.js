(() => {
  ...
  class OrchestratorApi {
    constructor({ orchestrator, planningEngine = null, schedulerEngine = null, reviewEngine = null, integrationEngine = null, recoveryController = null, eventBus = null, projectBundleService = null, persistenceInfo = null, observabilityService = null, taskControlService = null, contextStore = null, contextPackets = null, repositoryService = null } = {}) {
      this.orchestrator = orchestrator; this.planningEngine = planningEngine; this.schedulerEngine = schedulerEngine; this.reviewEngine = reviewEngine; this.integrationEngine = integrationEngine; this.recoveryController = recoveryController; this.eventBus = eventBus; this.projectBundleService = projectBundleService; this.persistenceInfo = persistenceInfo; this.observabilityService = observabilityService; this.taskControlService = taskControlService; this.contextStore = contextStore; this.contextPackets = contextPackets; this.repositoryService = repositoryService;
    }
    ...
    async query(name, payload = {}) {
      ...
      if (query === "dashboard") { const dashboard = this.dashboard(payload); return dashboard ? this.envelope({ ok: true, dashboard }) : this.envelope({ ok: false, reason: "observability_unavailable" }); }
      if (query === "taskGraph") { const dashboard = this.dashboard({ eventLimit: 1, decisionLimit: 1 }); return this.envelope({ ok: true, projectId: dashboard?.project?.projectId || null, tasks: dashboard?.tasks || [] }); }
      if (query === "taskDetails") { const task = this.observabilityService?.task?.(payload.taskId) || null; return task ? this.envelope({ ok: true, task }) : this.envelope({ ok: false, reason: "unknown_task", taskId: payload.taskId || null }); }
      ...
      if (query === "reviewDetails") { const dashboard = this.dashboard({ eventLimit: 1, decisionLimit: 1 }); const items = dashboard?.reviews?.items || []; const review = payload.reviewId ? items.find((item) => item.reviewId === payload.reviewId) || null : null; return payload.reviewId && !review ? this.envelope({ ok: false, reason: "unknown_review", reviewId: payload.reviewId }) : this.envelope({ ok: true, review, reviews: payload.reviewId ? undefined : items }); }
      if (query === "integrationEvidence") { const dashboard = this.dashboard({ eventLimit: 1, decisionLimit: 1 }); return this.envelope({ ok: true, integration: dashboard?.integration || null }); }
      if (query === "contextSummary") return this.envelope({ ok: true, context: this.contextStore?.summary?.() || null });
      if (query === "contextPacket") { if (!this.contextPackets?.packetForRole) return this.envelope({ ok: false, reason: "context_packets_unavailable" }); const result = this.contextPackets.packetForRole(payload); return this.envelope(result && typeof result === "object" ? result : { ok: false, reason: "context_packet_query_failed" }); }
      ...
      if (query === "persistence") { const info = typeof this.persistenceInfo === "function" ? this.persistenceInfo() : (this.persistenceInfo || {}); return this.envelope({ ok: true, persistence: { portableSchemaVersion: root.PortableState?.PORTABLE_SCHEMA_VERSION || 1, bundleVersion: root.ProjectBundle?.BUNDLE_VERSION || 1, contextPacketVersion: root.ContextPackets?.PACKET_VERSION || 1, ...info } }); }
      if (query === "repositories") { if (!this.repositoryService?.listRepositories) return this.envelope({ ok: false, reason: "repository_service_unavailable" }); return this.envelope(await this.repositoryService.listRepositories(payload)); }
      if (query === "repository") { if (!this.repositoryService?.getRepository) return this.envelope({ ok: false, reason: "repository_service_unavailable" }); return this.envelope(await this.repositoryService.getRepository(payload.repositoryId)); }
      if (query === "workspaceStatus") { if (!this.repositoryService?.workspaceStatus) return this.envelope({ ok: false, reason: "repository_service_unavailable" }); return this.envelope(await this.repositoryService.workspaceStatus(payload)); }
    ...
    async execute(name, payload = {}) {
      ...
      if (command === "startProject") {
        if (payload.repositoryId) {
          if (!this.repositoryService?.getRepository) result = { ok: false, reason: "repository_service_unavailable" };
          else {
            const linked = await this.repositoryService.getRepository(payload.repositoryId);
            if (!linked?.ok) result = { ok: false, reason: linked?.reason || "repository_not_registered", repositoryId: payload.repositoryId };
            else result = await this.planningEngine?.startProject?.({ goal: payload.goal, repositoryUrl: payload.repositoryUrl, repositoryId: payload.repositoryId });
          }
      ...
      else if (command === "importProjectBundle") { const recoveryStatus = String(this.recoveryController?.getPublicState?.()?.status || "IDLE"); if (!IMPORT_SAFE_RECOVERY_STATES.has(recoveryStatus)) result = { ok: false, reason: "portable_import_requires_safe_recovery_state", recoveryStatus }; else { result = await this.projectBundleService?.importBundle?.(payload.bundle, { replace: payload.replace === true, freezeAfter: true }); if (result?.ok) result = { ...result, reloadRequired: true }; } }
    ...
    async handleLegacyMessage(message, senderContext = {}) {
      const TYPES = root.MESSAGE_TYPES || {}; if (senderContext?.sessionId) return this.envelope({ ok: false, reason: "orchestrator_command_forbidden_from_agent_session" }); const payload = message?.payload || {}; const type = message?.type;
      if (type === TYPES.ORCHESTRATOR_API_QUERY) return this.query(payload.name, payload.payload || {}); if (type === TYPES.ORCHESTRATOR_API_EXECUTE) return this.execute(payload.name, payload.payload || {});
      ...
      if (queryMap.has(type)) return this.query(queryMap.get(type), payload); if (commandMap.has(type)) return this.execute(commandMap.get(type), payload); return this.envelope({ ok: false, reason: "unknown_runtime_message" });
    }
  }

  root.OrchestratorApi = OrchestratorApi; root.ORCHESTRATOR_API_VERSION = API_VERSION;
