(() => {
  ...
  class ServiceWorkerOrchestrator {
    ...
    async reconcileRegisteredTabs() {
      for (const agent of this.registry.listAgents()) {
        ...
        try {
          ...
          await this.registry.bindAgent(agent.agentId, {
            tabId: agent.tabId,
            chatUrl: tab.url || agent.chatUrl,
            status: "CONNECTING"
          });
    ...
    async registerActiveLead() {
      ...
      const tab = tabs?.[0];
      if (!tab || !Number.isInteger(tab.id) || !root.isChatGPTUrl(tab.url)) {
        return { ok: false, reason: "active_tab_is_not_chatgpt" };
      }

      const alreadyBound = this.registry.getAgentByTabId(tab.id);
      if (alreadyBound?.role === "worker") {
        return { ok: false, reason: "active_tab_is_worker", agentId: alreadyBound.agentId };
      }
      ...
      if (lead) {
        ...
        lead = await this.registry.createAgent({
          role: "lead",
          tabId: tab.id,
          chatUrl: tab.url,
          label: "Lead",
    ...
    async refreshAgentFromContent(agentId) {
      ...
      try {
        const response = await this.chrome.tabs.sendMessage(agent.tabId, {
          type: root.MESSAGE_TYPES.PING,
          payload: { agentId }
        });
    ...
    async sendPromptToAgent(agentId, prompt) {
      ...
      if (!agent || !Number.isInteger(agent.tabId)) return { ok: false, reason: "agent_offline" };
      try {
        const result = await this.chrome.tabs.sendMessage(agent.tabId, {
          type: root.MESSAGE_TYPES.SEND_PROMPT,
    ...
    async stopAgent(agentId) {
      ...
      try {
        const result = await this.chrome.tabs.sendMessage(agent.tabId, {
          type: root.MESSAGE_TYPES.STOP_GENERATION,
          payload: { agentId }
        });
        return { ...result, agentId };
      } catch (error) {
    ...
    async handleTabUpdated(tabId, changeInfo, tab) {
      ...
      if (changeInfo?.status === "complete" && root.isChatGPTUrl(tab?.url || changeInfo?.url)) {
        await this.refreshAgentFromContent(agent.agentId);
      }
    }
  }
  ...
  if (typeof module !== "undefined" && module.exports) {
    module.exports = { ServiceWorkerOrchestrator, MAX_WORKERS, CHATGPT_HOME };
  }
})();
